...
add_vars_list <- list(
  ...
  Biomass_production = list(
    ...
    sum_vars=c('FATES_LEAF_ALLOC','FATES_FROOT_ALLOC','FATES_CROOT_ALLOC','FATES_STEM_ALLOC','FATES_SEED_ALLOC')
  ),
  
  Biomass_production_incStore = list(
    name = 'Biomass_production_incStore',
    sum_vars=c('FATES_LEAF_ALLOC','FATES_FROOT_ALLOC','FATES_CROOT_ALLOC','FATES_STEM_ALLOC','FATES_SEED_ALLOC','FATES_STORE_ALLOC')
  ),
  
  FROOT_alloc_frac = list(
    name = 'FROOT_alloc_frac',
    sum_vars = c('FATES_FROOT_ALLOC'),
    div_sumvars=c('FATES_LEAF_ALLOC','FATES_FROOT_ALLOC','FATES_CROOT_ALLOC','FATES_STEM_ALLOC','FATES_SEED_ALLOC')
...
add_vars_list_cnp <- list(
  NUP = list(
    name = 'NUP',
    sum_vars=c('FATES_NH4UPTAKE','FATES_NO3UPTAKE')
  )
)
...
add_vars_lndtime <- function(a3d, vars_list, days=365, div_zero=1e-2, transpose=F ) {
  ...
  for(i in 1:length(vars_list)) {
    ...
    print('')
    print(paste('Processing:', vars_list[[i]]$name))
    print(vars_list[[i]]$sum_vars)
    ...
    print(paste('Completed:',aodimnames[[vss]][length(aodimnames[[vss]])]))
  }   
