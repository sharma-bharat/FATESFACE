...
for sim_case in sim_cases:
    ...
    for var in vars:
        for idx, key in enumerate(ds.keys()):
            ...
            if f"{select_site}" in key:
                ...
                plt.ylim(400, 1800)
                plt.title(
                    f"Annual NPP for {select_site} under {sim_case} using Cal-A parms\n",
                    fontsize=16,
                )
    ...
    plt.savefig(
        f"{path_save_results}/NPP_Best_Parms_Cal_{select_site}_{sim_case}.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    plt.savefig(
        f"{path_save_results}/NPP_Best_Parms_Cal_{select_site}_{sim_case}.png",
        format="png",
        dpi=200,
        bbox_inches="tight",
    )
    ...
    for var in vars:
        for idx, key in enumerate(ds.keys()):
            ...
            if f"{select_site}" in key:
                ...
                plt.ylim(0, 500)
                plt.title(
                    f"Annual NPP Response for {select_site} under {sim_case} using Cal-A parms\n",
                    fontsize=16,
                )
                # li+=1 # for legend index updating for a site
        ...
        # print (df)
        df.to_csv(
            f"{path_save_results}/DF_NPP_response_Cal_{select_site}_{sim_case}_{var}.csv",
            index=False,
        )
