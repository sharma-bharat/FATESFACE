...
# BSharma
# Oct 2023 
# Change lines 48-69 for input values
##############################
...
#caseidprefix_gen <- 'FACE_1PFT_r240731_dL2FR' 
caseidprefix_gen <- 'FACE_1PFT_r240826_BASE_CmEfAcnpEDphStoich_Log19'
caseidprefix_gen <- 'FACE_1PFT_r240821b_BASE_CmEfAcnpEDphStoich_Log19'
caseidprefix_gen <- 'FACE_1PFT_r240909_BASE_Log19_Selected'
caseidprefix_gen <- 'FACE_1PFT_r241009_BASE_Log19_StoreMax'
...
MF = 0 # [0 1]
Logging = "NewMet" # "Log" or "NoLog" 
Logging = "NewMet_EnVcmax"
Logging = "Store_max"

# Plotting
co2_exp = 'ELE' # 'AMB', 'ELE' choose either
...
for(c in caseidprefix) {
  ...
  for(s in sites) {
    ...
    for(co2 in cases) {
      ...
      # rl1 <- readRDS(paste0(file.path(root,moddate,paste0(c,'_processed')),'/',fname,'.RDS'))
...
labs <- list(
  ...
  NPP_MinusExcessResp = list(
    fvar_obs = 'NPP',
    base     = expression("NPP-ExResp [gC " * m^-2 * " y"^-1 * "]"),
    resp     = expression("NPP-ExResp response [gC " * m^-2 * " y"^-1 * "]"),
    rel      = expression("NPP-ExResp response [%]"),
    ylim     = NULL, #c(0,1900),
    ylim_abs = NULL, #c(0,500),
    ylim_rel = NULL #c(0,50)
  ),
...
figure_save_path = file.path(root,moddate,'figures',caseidprefix_gen, paste0(Logging,'_MF',MF,'_',co2_exp))
dir.create(file.path(root,moddate,'figures',caseidprefix_gen))
