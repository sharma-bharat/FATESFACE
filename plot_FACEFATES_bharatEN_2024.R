...
caseidprefix_gen <- 'FACE_1PFT_r240827_enBaseRate'
caseidprefix_gen <- 'FACE_1PFT_r240902_enVmax'
caseidprefix_gen <- 'FACE_1PFT_r240908_enBaseRate'
caseidprefix_gen <- 'FACE_r240912_enBaseRate'
caseidprefix_gen <- 'FACE_r240914c_enVmax'
...
for(c in caseidprefix) {
  ...
  for(s in sites) {
    ...
    for(co2 in cases) {
      ...
      # modifying single run for ensemble runs
      fname <- paste(c,s,co2,uq_member,'annual',sep='_')
      print(fname)
...
figure_save_path = file.path(root,moddate,'figures',caseidprefix_gen, paste0(Logging,'_MF',MF,'_',co2_exp,'_',uq_member))
